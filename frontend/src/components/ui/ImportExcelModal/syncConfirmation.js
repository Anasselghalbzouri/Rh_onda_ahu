...
export const buildSyncConfirmation = (result) => {
  ...
  const hasChanges = created + modified + deleted > 0

  const counts = `${created} créé(s), ${modified} modifié(s), ${unchanged} inchangé(s), ${deleted} supprimé(s)`
  ...
  return { hasChanges, created, modified, unchanged, deleted, message }
}
